...
GeoExt.Popup = Ext.extend(Ext.Window, {
     ...
     *  ``Boolean`` Position of the popup anchor (``bottom-left``, ``bottom-right``
    ...
    initComponent: function() {
        ...
        if (this.anchorPosition === "auto") {
            ...
            // horizontal
            if (centerPx.x > mapBox.width / 2) {
                horizontal = "right";
            } else {
                horizontal = "left";
            }

            this.anchorPosition = vertical + "-" + horizontal;
        }
    ...
    onRender: function(ct, position) {
        ...
        if (this.anchorPosition === "top-left" || this.anchorPosition === "top-right") {
            this.ancCls += " top";
        }
        if (this.anchorPosition === "bottom-right" || this.anchorPosition === "top-right") {
            this.ancCls += " right";
        }
    ...
    setSize: function(w, h) {
        ...
        // set the position of the anchor in case of bottom-right, bottom-left and top-left
        if (this.anc) {
	    // decide if render below or on top of the location
            if (this.anchorPosition === "bottom-right") {
                this.anc.setLeft(w - ancSize.width-10 + 'px');
            } else if (this.anchorPosition === "top-left") {
                this.anc.setTop(-(h == null ? this.getSize().height : h) + 'px');
            } else if (this.anchorPosition === "top-right") {
                this.anc.setLeft(w - ancSize.width-10 + 'px');
                this.anc.setTop(-(h == null ? this.getSize().height : h) + 'px');
            }
        }
    ...
    position: function() {
        ...
        if(this.isVisible()) {
            ...
            // below
            var ancWidth = anc.getWidth() / 2;

            // decide if render below or on top of the location
            if (this.anchorPosition === "bottom-left") {
                // on top
                this.setPosition(centerPx.x + mapBox.x - dx, 
                                 centerPx.y + mapBox.y - dy);
            } else if (this.anchorPosition === "bottom-right") {
                this.setPosition(centerPx.x - this.el.getWidth() / 2 - anc.getWidth() / 2, 
                                 centerPx.y + mapBox.y - dy);
            } else if (this.anchorPosition === "top-left") {
                this.setPosition(centerPx.x + mapBox.x - dx, 
                                 centerPx.y + mapBox.y + ancWidth);
            } else {
                this.setPosition(centerPx.x - this.el.getWidth() / 2 - anc.getWidth() / 2, 
                                 centerPx.y + mapBox.y + ancWidth);
            }
