...
GeoExt.grid.FeatureSelectionModelMixin = function() {
    return {
         ...
         *  ``OpenLayers.Layer.Vector`` A layer to store the selected features.
         */
        selectedFeatures: [],
        ...
        recenterToSelectionExtent: function() {
            ...
            var selectionExtentZoom = map.getZoomForExtent(selectionExtent, true);
            
            if(selectionExtentZoom > map.getZoom()) {
                map.setCenter(selectionExtent.lonlat);
            }
            else {
                map.zoomToExtent(this.getSelectionExtent());
            }
        },
